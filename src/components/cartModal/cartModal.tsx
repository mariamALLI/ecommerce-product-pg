...
export default function CartModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div
      className="fixed top-16 right-4 md:right-12 bg-white rounded-xl shadow-xl z-50 w-80 max-w-full"
      onClick={onClose}
    >
      <div className="p-4 border-b font-bold">Cart</div>
      <div className="p-4">
        {items.length === 0 ? (
          <div className="text-darkGrayishBlue font-semibold text-center py-10">
            Your cart is empty.
          </div>
        ) : (
          <>
            {items.map((item) => (
              <div key={item.id} className="flex items-center gap-2 mb-4">
                ...
                <button
                  ...
                  className="text-gray-400 hover:bg-red-500 text-xl cursor-pointer"
            ...
            <button className="w-full bg-orange text-white font-bold rounded-lg py-3 mt-2 cursor-pointer">
              Checkout
            </button>
          </>
        )}
